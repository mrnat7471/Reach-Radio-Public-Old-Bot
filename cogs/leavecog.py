        ...
        try:
            ...
            with open('config.json', 'r') as f:
                ...
                for user in log:
                    ...
                    await user.send(f"[Leave Command] Disconnected from Voice Channel in guild: {ctx.message.author.guild} ran by {ctx.message.author.name}({str(ctx.message.author.id)})")
            print(f"[Leave Command] Disconnected from Voice Channel in guild: {ctx.message.author.guild} ran by {ctx.message.author.name}({str(ctx.message.author.id)})")
